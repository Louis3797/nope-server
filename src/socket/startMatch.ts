...
import { getTournamentInfo } from 'src/service/tournament.service';
...
const startMatch = async (
  ...
  // Todo game loop: while game not ended
  setTimeout(() => {
    let isEnd = false;
    ...
    while (!isEnd) {
      ...
      currPlayer.socket.timeout(10000).emit(
        ...
        (err, response) => {
          if (err) {
            // Todo timeout means the other player wins the game
  ...
  if (gameWinner.points < bestOf / 2) {
    // if no winner for the match is certain
    // recursive call next game
    // eslint-disable-next-line @typescript-eslint/no-floating-promises
    startMatch(io, opponents, tournamentId, matchId, bestOf);
  } else {
    // else
    ...
    queue.enqueue(opponents.player1, opponents.player1.priority + 1);
    queue.enqueue(opponents.player2, opponents.player2.priority + 1);
  }
};
