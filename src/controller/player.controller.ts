...
export const getParticipatedMatches = async (
  ...
  const matches = await prismaClient.player.findUnique({
    where: {
      username: req.params.name
    },
    select: {
...
export const getWonMatches = async (
  ...
  const wonMatches = await prismaClient.player.findUnique({
    where: {
      username: req.params.name
    },
    select: {
  ...
  res.status(httpStatus.OK).json(wonMatches);
};
...
export const getParticipatedGames = async (
  ...
  const games = await prismaClient.player.findUnique({
    where: {
      username: req.params.name
    },
    select: {
